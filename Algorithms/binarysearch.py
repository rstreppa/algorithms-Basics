...
def isMajorityElement( nums, target ):
    """ 
        ...
        Find the first occurrence of target, you do so by requiring if nums[mid] >= target: r = mid
        which will shrink the seacrh interval to the left
        Finally once found the first occurence of target, check that l + len(nums)//2 is still equal to target and is within bounds
    """
